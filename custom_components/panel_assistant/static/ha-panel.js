...
class ye extends HTMLElement {
  ...
  constructor() {
    super(), this.attachShadow({ mode: "open" }), this.shadowRoot.innerHTML = `<style>
      ...
      <div id="spacer"></div>
      <a id="github" href="${se}" target="_blank" rel="noopener"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="${re}"/></svg></a>
    ...
    s.setAttribute("aria-label", I.github), s.setAttribute("title", I.github);
    for (const [a, r] of [["add", oe], ["settings", de]]) {
      const l = e.querySelector(`#${a}`);
      l.setAttribute("href", r), l.addEventListener("click", (d) => {
        d.defaultPrevented || d.button !== 0 || d.metaKey || d.ctrlKey || d.shiftKey || d.altKey || (d.preventDefault(), pe(r));
      });
    }
  ...
  set panel(e) {
    this.#d = e, this.shadowRoot.querySelector("#version").textContent = ue(e?.config);
  }
  get narrow() {
    return this.#e;
  }
  set narrow(e) {
    ...
    t.querySelector("#menu").hidden = !this.#e, t.querySelector("#title").hidden = this.#e, t.querySelector("#version").hidden = this.#e, t.querySelector("#github").hidden = this.#e, t.querySelector("#add-label").textContent = I[this.#e ? "addPanelShort" : "addPanel"];
  }
  connectedCallback() {
    clearInterval(this.#g), this.#y(), this.#g = setInterval(() => this.#A(), ce);
  }
  ...
  #M(e, t, n) {
    ...
    a.unsubscribe = Promise.resolve().then(() => s.connection.subscribeMessage((l) => this.#x(a, l), r, { resubscribe: !1 })), a.unsubscribe.catch((l) => {
      this.#t === a && (a.state = "failed", a.code = l?.code ?? null, this.#b(), this.#c());
    });
  }
  #x(e, t) {
    if (!(this.#t !== e || !t))
  ...
  #c() {
    ...
    const a = n.find((A) => A.entry_id === this.#s), r = this.#t, l = e.querySelector("#frame");
    let d = "", c = !1;
    this.#u() ? this.#n !== "ready" ? d = this.#n : r?.state === "closed" && r.entryId === a?.entry_id ? d = "closed" : a?.state === "unreachable" ? d = "unreachableBody" : a?.state === "not_loaded" ? d = "notLoadedBody" : r?.state === "failed" ? d = r.code === "not_loaded" ? "notLoadedBody" : "failed" : r?.state !== "open" && !l.getAttribute("src") && (c = !0) : d = "admin";
    const h = e.querySelector("#status");
    h.textContent = d ? I[d] : "", h.hidden = !d;
    const p = e.querySelector("#loading");
    p.hidden = !c, c && (e.querySelector("#loading-text").textContent = Ae(a?.title)), l.hidden = !l.getAttribute("src");
  }
}
customElements.get("panel-assistant-sidebar") || customElements.define("panel-assistant-sidebar", ye);
const Z = 64, J = 256 * 1024, Me = 2147483647, be = /^v(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)$/, me = /^v(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)-rc[1-9][0-9]*$/, Q = /^build-([1-9][0-9]{0,9})$/, xe = /^[0-9A-Za-z][0-9A-Za-z._+-]{0,63}$/, L = (i, e) => typeof e == "string" && e.length <= Z && i.exec(e)?.[0] === e, X = (i) => L(be, i), K = (i) => L(me, i);
function _(i) {
  if (!L(Q, i)) return null;
  const e = Number(Q.exec(i)[1]);
  return e <= Me ? e : null;
}
const O = (i) => _(i) !== null, we = (i) => L(xe, i), Ee = (i, e) => `${i} build ${e}`, P = "/api/panel_assistant/usb/release", R = 64 * 1024 * 1024, Ce = 1800 * 1e3, De = [
  "id",
  "tag",
  "checksum",
  "checksum_signature",
  ...
  "apk_size",
  "apk_sha256"
], ve = ["id", "tag", "feed", "feed_signature", "apk_size", "apk_sha256"], $ = 8192, je = Math.ceil(J / 3) * 4 + $, j = (i, e) => typeof e == "string" && i.exec(e)?.[0] === e, H = (i, e) => i !== null && typeof i == "object" && !Array.isArray(i) && Object.keys(i).length === e.length && e.every((t) => Object.hasOwn(i, t));
...
async function G(i, e, t, n = null) {
  ...
  if (s !== null) {
    u(j(/0|[1-9][0-9]*/, s));
    const c = Number(s);
    u(Number.isSafeInteger(c) && c <= e && (n === null || c === n));
  }
  ...
  try {
    for (; ; ) {
      ...
      if (u(!t.aborted, "cancelled"), c.done) break;
      d += c.value.byteLength, u(d <= e && (n === null || d <= n)), l.push(c.value);
    }
    return u(d > 0 && (s === null || d === Number(s)) && (n === null || d === n)), new Blob(l);
  } finally {
...
function Le(i, e, {
  ...
  let r, l;
  const d = new Promise((o, b) => {
    r = o, l = b;
  }), c = new AbortController();
  let h = !1, p, A, f, C, N = !1, S = !1, w, k, B;
  const D = () => {
    clearInterval(k), clearTimeout(B), w = void 0, s.removeEventListener("message", T);
  }, E = (o) => {
    try {
      n(o);
    } catch {
    ...
    if (!h) {
      if (h = !0, c.abort(), clearTimeout(A), E(o ?? "verified"), o) {
        D(), l(new v(o));
        return;
      }
      k = setInterval(() => {
        p.closed && D();
      }, 2e3), B = setTimeout(D, Ce), r();
    }
  };
  async function ee() {
    try {
      ...
      u(!h, "cancelled"), u(H(g, b ? ve : De) && j(/[0-9a-f]{32}/, g.id) && typeof g.tag == "string" && g.tag.length <= Z && (t === null ? X(g.tag) : g.tag === t) && j(/[0-9a-f]{64}/, g.apk_sha256) && Number.isSafeInteger(g.apk_size) && g.apk_size > 0 && g.apk_size <= R);
      const te = b ? {
        tag: g.tag,
        feed: x(g.feed, J),
      ...
      u(!h, "cancelled");
      const ne = await G(ie, R, c.signal, g.apk_size);
  ...
  try {
    u(i && typeof i.fetchWithAuth == "function" && (t === null || K(t) || O(t)) && Number.isSafeInteger(a) && a > 0 && a <= 3e5, "invalid_request");
    const o = new URL(e);
    ...
    s.crypto.getRandomValues(b), C = Array.from(b, (g) => g.toString(16).padStart(2, "0")).join(""), o.hash = new URLSearchParams({ ha_origin: s.location.origin, nonce: C, rc: t ?? "" }).toString(), s.addEventListener("message", T), p = s.open(o.href, "_blank"), u(p, "popup_blocked"), A = setTimeout(() => m("timeout"), a), E("waiting");
  } catch (o) {
    m(o instanceof v ? o.code : "invalid_request");
  }
  return { completion: d, cancel: () => {
    m("cancelled"), D();
  } };
}
const q = 30, Y = 500, U = 128 * 1024, z = (i, e) => i !== null && typeof i == "object" && !Array.isArray(i) && Object.keys(i).length === e.length && e.every((t) => Object.hasOwn(i, t));
function y(i) {
...
function Ne(i) {
  y(z(i, ["releases"]) && Array.isArray(i.releases) && i.releases.length <= q + Y);
  const e = /* @__PURE__ */ new Set();
  let t = 0, n = 0, s = 0;
  return Object.freeze(i.releases.map((a) => z(a, ["tag", "prerelease", "name"]) ? (y(ze(a) && !e.has(a.tag) && ++s <= Y), e.add(a.tag), Object.freeze({ tag: a.tag, prerelease: !0, name: a.name })) : (y(z(a, ["tag", "prerelease"]) && typeof a.prerelease == "boolean" && (a.prerelease ? K(a.tag) : X(a.tag)) && !e.has(a.tag) && ++n <= q), e.add(a.tag), a.prerelease || y(++t <= 1), Object.freeze({ tag: a.tag, prerelease: a.prerelease }))));
}
async function Se(i, { signal: e, timeoutMs: t = 15e3 } = {}) {
  ...
  let r, l;
  const d = new Promise((c, h) => {
    l = () => h(new Error("Release catalogue cancelled"));
  });
  n.signal.addEventListener("abort", l, { once: !0 });
  try {
    return y(!n.signal.aborted), await Promise.race([d, (async () => {
      const c = await i.fetchWithAuth("/api/panel_assistant/usb/releases", {
        method: "GET",
        redirect: "error",
        signal: n.signal
      });
      y(!n.signal.aborted && c.status === 200 && !c.redirected && c.body && c.headers.get("content-type")?.split(";")[0].trim() === "application/json");
      const h = c.headers.get("content-length");
      y(h === null || /^(0|[1-9][0-9]*)$/.exec(h)?.[0] === h && Number(h) <= U), r = c.body.getReader();
      const p = [];
      let A = 0;
      for (; ; ) {
        const f = await r.read();
        if (y(!n.signal.aborted), f.done) break;
        A += f.value.byteLength, y(A <= U), p.push(f.value);
      }
      return y(A > 0 && (h === null || A === Number(h))), Ne(JSON.parse(await new Blob(p).text()));
    })()]);
  } finally {
...
class Oe extends HTMLElement {
  ...
  async #s() {
    ...
    try {
      ...
      s.value = "", s.textContent = M.choose, s.disabled = !0, n.append(s);
      const a = t.find((r) => !r.prerelease)?.tag ?? "";
      for (const r of t) {
        const l = document.createElement("option");
        l.value = r.tag;
        const d = r.tag === a ? M.recommended : r.name ? M.devBuild : r.prerelease ? M.testing : "";
        l.textContent = `${r.name ?? r.tag.replace(/^v/, "")}${d ? ` (${d})` : ""}`, n.append(l);
      }
      n.value = a;
    } catch {
      if (this.#n !== e) return;
  ...
  #t() {
    ...
    s.textContent = e && t && this.#a !== "ready" ? M[this.#a] : "", s.hidden = !s.textContent, this.shadowRoot.querySelector("#retry").hidden = !e || !t || !["catalogError", "empty"].includes(this.#a), this.shadowRoot.querySelector("#cancel").hidden = !this.#e;
    const a = e ? t ? this.#o : "unavailable" : "admin", r = this.shadowRoot.querySelector("#status");
    r.textContent = Object.hasOwn(M, a) ? M[a] : M.failed, r.hidden = !r.textContent;
  }
  #g() {
